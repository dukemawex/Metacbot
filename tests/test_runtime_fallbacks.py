...
class StubExaClientWithSearchFailure:
    def search(self, _query: str) -> list[dict]:
        raise RuntimeError("boom")


class StubLLMClientWithChatFailure:
    def chat_json(self, _prompt: str) -> dict:
        raise ValueError("bad")


class StubExaClientWithEmptyResults:
    def search(self, _query: str) -> list[dict]:
        return []


def test_retrieve_evidence_handles_missing_id():
    evidence = retrieve_evidence({"title": "Test", "id": None}, StubExaClientWithEmptyResults())
    assert evidence.question_id == 0
    assert evidence.items == []


def test_retrieve_evidence_handles_search_failures():
    evidence = retrieve_evidence({"title": "Test", "id": 42}, StubExaClientWithSearchFailure())
...
def test_run_roles_returns_empty_payloads_on_failure():
    ...
    outputs = run_roles({"id": 123, "title": "Test"}, evidence, StubLLMClientWithChatFailure())
